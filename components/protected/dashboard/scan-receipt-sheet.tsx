...
export function ScanReceiptSheet({ isOpen, onClose, onParsed }: ScanReceiptSheetProps) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    console.log('[ScanReceipt] start submit, files:', files.map(f => ({ name: f.name, type: f.type, size: f.size })))
    ...
    try {
      ...
      for (const file of files) {
        ...
        console.log('[ScanReceipt] uploading file to storage:', { bucket: 'receipts', path })
      ...
      const res = await fetch('/api/v1/ocr-receipt', { method: 'POST', body: fd })
      if (!res.ok) {
    ...
    <Sheet open={isOpen} onOpenChange={handleClose}>
      <SheetContent className="flex flex-col gap-0 p-0 sm:max-w-2xl">
        <SheetHeader className="p-6 border-b">
          <SheetTitle className="text-xl font-semibold">Nowy skan paragonu</SheetTitle>
          <SheetDescription>Dodaj zdjęcia paragonu. System odczyta dane przez OCR.</SheetDescription>
        </SheetHeader>

        <div className="flex-1 overflow-y-auto px-6 py-6">
          <form id="scan-receipt-form" onSubmit={onSubmit} className="space-y-6">
        ...
        <SheetFooter className="mt-auto border-t p-6 flex justify-end gap-3">
          ...
          <Button type="submit" form="scan-receipt-form" disabled={isBusy}>
            ...
            {isUploading ? 'Wgrywanie…' : isProcessing ? 'Przetwarzanie OCR…' : 'Skanuj'}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
