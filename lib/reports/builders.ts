...
type Row = { id: string; date: string; description: string; category: string; amount: number; currency: string }
...
export async function buildCsvBuffer(rows: Row[]): Promise<Buffer> {
    ...
    const lines = [header.join(","), ...rows.map(r => [
        r.date, q(r.description), q(r.category), r.amount.toFixed(2), r.currency
    ].join(","))]
...
export async function buildDocxBuffer(input: DocxInput): Promise<Buffer> {
    ...
    const headerRow = new TableRow({
        children: ["Date", "Description", "Category", "Amount", "Currency"].map(h =>
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: h, bold: true })] })] })
        ),
    })
    const bodyRows = input.rows.map(r => new TableRow({
        children: [
            r.date, r.description, r.category, r.amount.toFixed(2), r.currency,
    ...
    const doc = new Document({
        sections: [{
            children: [
                ...
                new Paragraph(`Total: ${total.toFixed(2)} ${input.rows[0]?.currency || ""}`),
                table,
            ],
        }],
    })
