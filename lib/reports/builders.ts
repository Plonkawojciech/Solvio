...
export async function buildPdfBuffer(input: PdfInput): Promise<Buffer> {
  ...
  pages.forEach((p, idx) => {
    drawText(p, `Generated by Solvio  /  solvio.programo.pl  /  Page ${idx + 1} of ${pages.length}`,
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `Generated by Solvio  /  solvio.programo.pl  /  ${new Date().toISOString().slice(0, 10)}`,
                font: 'Courier New',
                size: 16,
                color: '999999',
              }),
