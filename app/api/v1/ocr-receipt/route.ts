...
export async function POST(req: NextRequest) {
  ...
  for (const f of files) {
    try {
      ...
      const res = await processImage({ buffer, filename: f.name, mime: f.type })
  ...
  const picked =
    receipts.find(r => typeof r.total === 'number' && !Number.isNaN(r.total)) ??
    receipts[0] ??
    { vendor: null, date: null, total: null, currency: null, items: [], ocrText: '' }
...
async function processImage(opts: { buffer: Buffer; filename: string; mime: string }): Promise<OcrResponse> {
  ...
  if (!/^image\//i.test(mime)) {
    return {
      receipt_id: 'unknown',
      ocrText: '',
      parsed: { vendor: null, date: null, total: null, currency: null, items: [], ocrText: '' },
  ...
  const completion = await openai.chat.completions.parse({
    ...
    messages: [
      ...
      },
      {
        role: 'user',
        content: [
          { type: 'text', text: 'Extract structured receipt fields and the full OCR text from this image.' },
